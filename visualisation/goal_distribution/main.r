...
plot_goal_for <- ggplot(data_goal_for, aes(x=minute, color=goal_for)) + 
    ...
    theme_ipsum_rc() + 
    labs(x="Minutes", y="Density",title="Goals distribution",subtitle="Premier League 2018-2019",caption="by @Ben8t",color="") +
...
plot_goal_against <- ggplot(data_goal_against, aes(x=minute, color=goal_against)) + 
    ...
    theme_ipsum_rc() + 
    labs(x="Minutes", y="Density",title="Conceded goals distribution",subtitle="Premier League 2018-2019",caption="by @Ben8t",color="") +
    theme(text=element_text(colour="white"),
        plot.background=element_rect(fill="black"), 
        axis.text=element_text(colour="white"), 
        legend.text=element_text(colour="white"))
