...
raw_data <- read_csv("mu_xg.csv")
id = 32
subtitle = "Manchester United - 2015 to 2017 Premier League season"

data <- raw_data %>%
    filter(team_id==id) %>%
    group_by(startDate) %>%
    summarise(goal=sum(is_goal, na.rm=TRUE), xg=sum(xG, na.rm=TRUE)) %>%
...
ggplot() + 
    geom_line(data=grouped_data, aes(x=id, y=rollmean(xg, 15, na.pad=TRUE)), color="#56FFAE") +
    geom_point(data=grouped_data, aes(x=id, y=rollmean(xg, 15, na.pad=TRUE)), color="#56FFAE") +
    geom_line(data=grouped_data, aes(x=id, y=rollmean(goal, 15, na.pad=TRUE)), color="#FCA337") +
    geom_point(data=grouped_data, aes(x=id, y=rollmean(goal, 15, na.pad=TRUE)), color="#FCA337") + 
